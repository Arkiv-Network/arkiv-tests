# Arkiv single-validator PoS devnet (reth + Lighthouse)

An alternative to the `prod/` `--dev` sequencer that runs **without `--dev`**:
execution (arkiv reth) is driven over the Engine API by a real consensus layer
— a single-validator **Lighthouse** beacon node + validator client.
...
Endpoints (default offset ports, so this coexists with the `prod/` `--dev`
chain):
- EL RPC:     http://127.0.0.1:18545
- EL WS:      ws://127.0.0.1:18546
- EL metrics: http://127.0.0.1:16160
- Beacon API: http://127.0.0.1:15052
...
curl -s http://127.0.0.1:15052/eth/v1/node/syncing
curl -s http://127.0.0.1:15052/eth/v1/beacon/states/head/finality_checkpoints
```
...
- Prefunded accounts come from the genesis-generator mnemonic
  (`EL_AND_CL_MNEMONIC`), not `prod/test-accounts.txt`. If you need those exact
